...
from bilevel_poison_radius_gaussianaug import bilevel_poisoning
...
x_val_tf = tf.placeholder(tf.float32, shape=(100, height,width,nch))
...
bl_poisoning = bilevel_poisoning(sess, x_train_tf, x_val_tf, x_test_tf, x_poisoned_tf, x_original_tf, y_train_tf, y_val_tf, y_val_class_tf, y_test_tf, y_poisoned_tf, sigma_tf,
                                 batch_size_poisoned, height, width, nch, nclass, 100, batch_size_clean, 
...
for epoch in range(1,nepochs):
    ...
    for batch in range(nb_batches):
        ...
        ind_val = np.random.choice(len(X_val), size=(int(100)),replace=False)
