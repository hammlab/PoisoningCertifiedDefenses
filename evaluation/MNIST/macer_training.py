...
x_train_tf_reshaped = tf.reshape(x_train_tf, [-1, height*width*nch])
...
unsmoothed_outputs = KerasModelWrapper(model).get_logits(x_train_tf)
cls_test = KerasModelWrapper(model).get_logits(x_test_tf)

#Classification Loss on unsmoothed
ce_loss_unsmoothed = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=unsmoothed_outputs, labels=y_train_tf))
